...
<#
.SYNOPSIS
    Verifies an installed Loop product profile against docs/product-surface.json.
...
param(
    [string]$InstallDir = "",
    [ValidateSet("developer", "loop-release")]
    [string]$Profile = "loop-release",
    [string]$ManifestPath = "",
    [string]$BuildDir = "",
...
if ($null -eq $python) {
    throw "Python 3 is required to verify the Loop product surface."
}
