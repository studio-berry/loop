...
CMAKE_VERSION = re.compile(
    r"^set\(PDF4QT_VERSION\s+([0-9]+(?:\.[0-9]+)*)\)\s*$", re.MULTILINE
)
CMAKE_PRERELEASE = re.compile(
    r"^set\(PDF4QT_VERSION_PRERELEASE\s+([^)]*)\)\s*$", re.MULTILINE
)
FOUR_PART_RELEASE_GREP = re.compile(
    r"set\\\(PDF4QT_VERSION \\K\[0-9\]\+\\.\[0-9\]\+\\.\[0-9\]\+\\.\[0-9\]\+"
)
THREE_PART_RELEASE_GREP = re.compile(
    r"set\\\(PDF4QT_VERSION \\K\[0-9\]\+\\.\[0-9\]\+\\.\[0-9\]\+"
)
PRERELEASE_RELEASE_GREP = re.compile(r"PDF4QT_VERSION_PRERELEASE")
...
def parse_cmake_version(text: str) -> str:
    ...
    if not match:
        raise ValueError("CMakeLists.txt has no set(PDF4QT_VERSION ...) assignment")
    version = match.group(1)
    if not SEMVER_CORE.fullmatch(version):
        raise ValueError(
            f"PDF4QT_VERSION must be SemVer MAJOR.MINOR.PATCH without a fourth "
...
def validate_versioning_doc(text: str, policy: dict[str, str]) -> list[str]:
    ...
    if canonical and policy["cmake_variable"] not in canonical:
        errors.append("VERSIONING.md Canonical version must name PDF4QT_VERSION")
    ...
    if windows and policy["windows_variable"] not in windows:
        errors.append("VERSIONING.md Windows Appx version must name PDF4QT_WINDOWS_VERSION")
...
def validate_release_workflow(text: str, prerelease: str = "") -> list[str]:
    ...
    if FOUR_PART_RELEASE_GREP.search(text):
        errors.append(
            "CreateReleaseDraft.yml still parses a four-part PDF4QT_VERSION; "
            "use MAJOR.MINOR.PATCH"
        )
    elif not THREE_PART_RELEASE_GREP.search(text):
        errors.append(
            "CreateReleaseDraft.yml must grep set(PDF4QT_VERSION) as MAJOR.MINOR.PATCH"
        )
    if prerelease and not PRERELEASE_RELEASE_GREP.search(text):
        errors.append(
            "CreateReleaseDraft.yml must read PDF4QT_VERSION_PRERELEASE when a "
...
def validate_appx_manifest(text: str, policy: dict[str, str]) -> list[str]:
    ...
    if "${PDF4QT_VERSION}" in text:
        return ["AppxManifest.xml.in must not use three-part PDF4QT_VERSION"]
    return []
...
def validate_repository(root: Path) -> list[str]:
    ...
    if version is not None and version != policy["current"]:
        errors.append(
            f"PDF4QT_VERSION {version!r} must match version-policy.json current "
            f"{policy['current']!r}"
        )
    if prerelease != policy["prerelease"]:
        errors.append(
            f"PDF4QT_VERSION_PRERELEASE {prerelease!r} must match version-policy.json "
