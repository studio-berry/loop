...
class WorkflowContractTests(unittest.TestCase):
    ...
    def test_package_workflows_require_and_record_exact_source_sha(self):
        ...
        self.assertIn("cmake --build build --target LoopEditor PdfTool ProductQuickAccessibilitySmoke LoopLibQuickplugin release_translations -j6", linux)
        ...
        self.assertIn("[System.IO.File]::WriteAllText", windows)
        self.assertIn('Join-Path $installBin "qt.conf"', windows)
        self.assertNotIn("          [Paths]", windows)
        self.assertNotIn('@"', windows)
        self.assertIn("build\\LoopEditor\\Loop\\Quick", windows)
        self.assertIn("LoopLibQuick\\Loop\\Canvas", windows)
        self.assertIn("--qmldir (Join-Path $buildDir \"LoopEditor\\Loop\\Quick\")", windows)
        self.assertIn("Copy-BuiltSharedQmlPlugin", windows)
        self.assertIn("LoopLibQuickplugin release_translations", windows)
        self.assertNotIn("LoopEditorQuickplugin.dll", windows)
        self.assertNotIn("Copy-BuiltQmlPlugin", windows)
        self.assertIn("qoffscreen.dll", windows)
        self.assertIn("plugins\\sqldrivers\\qsqlite.dll", windows)
        self.assertIn("sqldrivers", windows)
        self.assertIn("qsqlite.dll", windows)
        ...
        self.assertIn('install/usr/bin/qt.conf', linux)
        self.assertIn("printf '%s\\n'", linux)
        self.assertIn("'[Paths]'", linux)
        self.assertNotIn("<<'EOF'", linux)
        self.assertNotIn("          [Paths]", linux)
        self.assertIn("Qml2Imports=lib/qml", linux)
        ...
        self.assertIn("cmake --build build --target LoopEditor PdfTool ProductQuickAccessibilitySmoke LoopLibQuickplugin release_translations --config Release -j6", windows)
