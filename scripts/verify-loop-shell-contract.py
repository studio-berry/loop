#!/usr/bin/env python3
"""Verify Loop shell contract (Python twin for Linux agents without pwsh)."""
...
ACTION_POLICY_PATH = ROOT / "docs" / "loop-shell-actions.json"
...
def main() -> int:
    ...
    if product_surface.get("shell_contract") != "docs/loop-shell.json":
        raise ContractError("product-surface manifest is not linked to loop-shell.json")
    ...
    if shell.get("shell_surface") != "LoopEditor":
        raise ContractError("shell_surface must be LoopEditor")
    ...
    print(
        f"Loop shell contract verified: {len(shell['workspaces'])} workspaces, "
...
if __name__ == "__main__":
    try:
        ...
        print(f"Loop shell contract FAILED: {exc}", file=sys.stderr)
        raise SystemExit(1)
