...
<#
.SYNOPSIS
    Verifies the non-visual Loop shell contract and Editor action policy.
...
param(
    ...
    [string]$ShellContractPath = (Join-Path $PSScriptRoot "..\docs\loop-shell.json"),
    [string]$ActionPolicyPath = (Join-Path $PSScriptRoot "..\docs\loop-shell-actions.json")
)
...
if ($productSurface.shell_contract -ne "docs/loop-shell.json") {
...
if ($shell.shell_surface -ne "LoopEditor") {
    throw "Loop shell must remain LoopEditor: $($shell.shell_surface)"
}
...
$expectedWorkspaceIds = @(
    "loop-document",
    "loop-preflight",
    "loop-production-preview",
    "loop-pages-production",
    "loop-inspect",
    "loop-fix",
    "loop-compare"
)
...
Write-Output "Loop shell contract verified: $($shell.workspaces.Count) workspaces, $($policyActions.Count) Editor actions, $($shell.plugin_action_policy.Count) plugin policies, $($legacyLedger.Count) legacy UI dispositions; $guiMessage"
