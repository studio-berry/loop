...
| `jbig2-zero-export-run-timeout.bin` | libFuzzer timeout (1201s), reproduced by hosted CI on the same 97-byte regression seed | `PDFJBIG2Decoder::processSymbolDictionary` can repeatedly decode zero-length `EXRUNLENGTH` values; each run inserts no bits, so the export-flags loop never grows `EXFLAGS` toward `symbolsSize`. | Bound consecutive empty runs at 1000 and compare each run against the remaining flag count before inserting. |

The first two seeds now execute in well under 100ms (was: 1200s+ and 30s+
respectively). The zero-export-run seed now terminates with a malformed-stream
error after the empty-run limit. See `docs/V1_RELEASE_READINESS.md` §R-003 for the full
