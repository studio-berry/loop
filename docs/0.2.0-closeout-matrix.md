...
| P-01 | Cross-platform/package | Linux/Windows native/software smoke, clean-machine package, QML deployment | **Open (re-admit)** — `b47c62b2…` inspector pair withdrawn: native a11y ran through software graphics and Windows smoke skipped Editor launch. Re-prove by dispatching `Linux_AppImage` + `Windows_MSI` + `Create_Release_Draft` with `operator_evidence=true` (stable promotion only). Default package dispatches stay fast. Windows Server 2022 pristine-VM remains deferred to 1.0. See `docs/SESSION_07_PACKAGE_BOUNDARY.md` |
