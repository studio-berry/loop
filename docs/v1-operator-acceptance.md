...
- Runs `PdfTool preflight` on each corpus file with `profiles/loop-default.json`
...
Run on a supported Windows or Linux machine with a release or dev build that bundles `PdfTool` and `share/loop/profiles/loop-default.json` (see [PLATFORM_SUPPORT.md](PLATFORM_SUPPORT.md)).

### Open and preflight

1. Launch **LoopEditor** and open `loop-preflight/testdata/fixtures/bleed-missing.pdf`.
2. **Loop Preflight → Run Preflight**.
