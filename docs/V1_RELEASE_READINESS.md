...
|--------|-----------|------|
| Linux CI | `.github/workflows/ci.yml` | Ubuntu build + `ctest` (no packaging artifact — `ci.yml` builds and tests only) |
...
| A12 | Installer | Clean-machine install (**Windows + Linux**) | **Fail / open** | MIC-301 In Review; harness at `scripts/Invoke-MsiSmokeTest.ps1`, clean-VM run outstanding. Linux AppImage smoke added to MIC-301 on 2026-07-25 — previously ungated. **`.deb` does not exist** — the earlier claim that it "ships in `CreateReleaseDraft.yml`" was wrong; that workflow only downloads and attaches artifacts from `LinuxInstall.yml` (AppImage) and `LinuxFlatpak.yml` (Flatpak), neither of which builds a `.deb` |
