...
| Overprint disclosure (MIC-330) | **Pass** | `overprintDisclosureText()` always shown; README + runbook R-002; `tst_preflightplugintest.cpp`. Independently reinforced by the canvas fidelity indicator (gh-49, PR #395 on `0.2.0-minor-fixes`) — see §3 R-002 |
...
| **R-002** | MIC-320 — overprint not simulated in standard page rendering | **Accepted — mitigation complete** | [MIC-330](https://linear.app/mbx2/issue/MIC-330) | Documented limitation + in-app disclosure, now at two levels. **Preflight** (unchanged): the report panel shows the general "page view does not simulate overprint" notice unconditionally once a report is loaded, with an additional specific warning appended only when a `white-overprint` finding is present. **Canvas** (added by gh-49 / PR #395 on `0.2.0-minor-fixes`, reviewed 2026-08-30 — not yet on `dev`): a persistent, non-modal fidelity indicator now appears directly on the canvas, independent of running preflight, whenever the current page's cached `PDFPrecompiledPage::containsOverprint()` flag is set; it names the reason and offers one-click escalation of just that page to the authoritative `PDFTransparencyRenderer` render (`PDFRenderPolicy::forOutputPreview()`), verified to match the same golden the Output Preview / preflight measurement path already trusts (see `docs/RENDERER_DIFFERENTIALS.md`). Escalation is opt-in per page — the canvas still renders overprint approximately by default — so this strengthens MIC-330's disclosure without reopening MIC-320's deferral. Deferral of MIC-320 is recorded in Linear (project description + MIC-306), not only here — see §3 |
...
| Question | Decision |
|----------|----------|
| Overprint-correct rendering a V1 gate? | **No** — deferred post-V1 (MIC-320). V1 ships detection + disclosure, no overprint-safe claim. The canvas now also offers an opt-in, one-click accurate render of the current page without leaving it (gh-49); the default render is still approximate, so this does not reopen the deferral |
...
| A13 | Overprint rendering | Correct overprint compositing in standard page view | **Deferred — mitigation complete** | MIC-320 deferred post-V1: standard page view still renders overprint approximately by default. `overprintDisclosureText()` always shown once a report loads; additional white-overprint warning when finding present (MIC-330). Canvas now also surfaces a persistent per-page fidelity indicator with one-click escalation to the authoritative render, independent of preflight (gh-49 / PR #395 on `0.2.0-minor-fixes`; see `docs/RENDERER_DIFFERENTIALS.md` and §3 R-002). README + runbook R-002 published |
...
| **R-002** | Page view does not simulate overprint | Print/prepress shops proofing overprint work | Open an overprint fixture; compare page view against Output Preview | Overprint is implemented in `pdftransparencyrenderer.cpp` (Output Preview) but absent from the standard QPainter path in `pdfpainter.cpp`; that renderer is RGB and overprint is a subtractive CMYK model, so correct handling there is an XL change (MIC-320) | **Accepted for V1:** documented limitation + in-app disclosure at two levels. The preflight report panel always tells the operator to use Output Preview once a report is loaded (not only when a `white-overprint` finding is present), with an additional specific warning for the unsafe white/near-white case. Independent of preflight, the canvas itself now shows a persistent fidelity banner on any page whose cached `PDFPrecompiledPage::containsOverprint()` flag is set, and lets the operator escalate that one page to the authoritative `PDFTransparencyRenderer` render (`PDFRenderPolicy::forOutputPreview()`) without reopening the document (gh-49 / PR #395 on `0.2.0-minor-fixes`, reviewed against this register 2026-08-30 — not yet merged to `dev`; see `docs/RENDERER_DIFFERENTIALS.md`). Escalation is opt-in per page — the default canvas render still approximates overprint, so MIC-320 remains deferred. README documents the general limitation. No "overprint-safe output" claim in marketing | Run preflight on any PDF; confirm the general panel note always appears. Run on `white-overprint-form.pdf`; confirm the additional specific warning also appears; proof via Output Preview. Separately: open a PDF with overprinted content on the canvas, confirm the fidelity banner appears, toggle it, and confirm the page now matches Output Preview (`UnitTestsPageSurface` asserts this against the same `overprint-cmyk-mode1-on.png` golden `UnitTestsOverprintRender` checks) | Product |
