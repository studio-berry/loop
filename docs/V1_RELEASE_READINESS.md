...
| ~~R-013~~ | ~~Fixup advertisement can drift between Editor and CLI~~ | MIC-338 | **Resolved.** The shipped Loupe Default profile contains `rgb-to-cmyk`, `add-bleed`, and `downsample-images`. Each advertised fixup must be implemented in the shared Core registry, applicable to the finding/document, and present in the active profile. Editor filtering and `PdfTool capabilities --console-format json` both use the shared registry; regression coverage checks the shipped profile. |
