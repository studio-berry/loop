...
1. Root CMake must not require `Qt6::Widgets` for the Loupe release profile. **Done** — `LOUPE_LOUPE_DISTRIBUTION=ON` defaults developer Widgets tools off and gates `find_package(Widgets)`; verified by `scripts/verify-widgets-free-release-profile.py`.
