...
**Debug files:** Windows CI uploads Loop PDBs to `berry-studios` project `4511866328449104` (EU) when `SENTRY_AUTH_TOKEN` is set. Without those files, crash stacks stay unsymbolicated. Store the token as a GitHub Actions secret with `project:releases` (or broader) scope; do not commit it.
