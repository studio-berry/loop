...
1. `LOUPE_OCR_SIDECAR` environment variable
2. Bundled `LoupeOcrService/LoupeOcrService.exe` next to `PdfTool`
3. Dev launcher `loupe-ocr/tools/dev_ocr_sidecar.cmd` (relative to a build-tree `PdfTool`)

```powershell
PdfTool ocr scan.pdf --console-format json --sidecar loupe-ocr\tools\dev_ocr_sidecar.cmd
```
