...
Standalone preflight **engine** for Loop (sidecar CLI). Lives outside the LOOP / Loop Editor process. The Qt plugin (Phase 2) shells out to this tool and only consumes the normalized report JSON defined here.
...
| `profiles/loop-default.yaml` | Bundled **Loop Default** profile (Venue Poster / Handbill / Signage) |
...
```bash
PdfTool preflight document.pdf --profile loop-preflight/examples/profile-pdfx-x4.json
```
...
Loop supports two tiers of bleed validation:
...
This check is deliberately not enabled by `loop-default.json`: it requires a
...
The shipped `profiles/loop-default.json` and its YAML authoring mirror list
...
PdfTool preflight document.pdf --profile loop-preflight/profiles/loop-default.json

# Tiered bleed profile (Tier-1 + Tier-2 content-bleed, fast bounds only)
PdfTool preflight document.pdf --profile loop-preflight/examples/profile-tiered-bleed.json

# Tiered bleed with strip raster confirmation (bleed-margin-empty findings)
PdfTool preflight document.pdf --profile loop-preflight/examples/profile-tiered-bleed-raster.json
```
...
3. Create its snapshot once locally: `LOOP_UPDATE_SNAPSHOTS=1 ctest -R UnitTestsPreflightCorpus`,
...
```bash
cmake --build build --target LoopGenerateFixtures
./build/usr/bin/LoopGenerateFixtures loop-preflight/testdata/fixtures
LOOP_UPDATE_SNAPSHOTS=1 ctest --test-dir build -R UnitTestsPreflightCorpus
```
...
Loop Default's custom rules. The custom-check fixtures live in this same corpus. Each is
...
|---------|---------|--------|----------|
| `color-rgb.pdf` | loop-default | fail | `color-mode` (DeviceRGB image) |
| `color-cmyk.pdf` | loop-default | pass | `color-mode` (DeviceCMYK image) |
| `image-dpi-low.pdf` | loop-default | warning | `image-resolution` (~25 DPI) |
| `image-dpi-ok.pdf` | loop-default | pass | `image-resolution` (~310 DPI) |
| `image-dpi-excessive.pdf` | loop-default | pass | 600-DPI image advertises `downsample-images` |
| `font-not-embedded.pdf` | loop-default | fail | `embedded-fonts` (Helvetica, no FontFile) |
| `font-embedded.pdf` | loop-default | pass | `embedded-fonts` (`/FontFile2` subset) |
...
Letter size at error severity (the shipped `loop-default` profile intentionally leaves the
expected size unset).
