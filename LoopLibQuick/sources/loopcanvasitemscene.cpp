...
QSGNode* LoopCanvasItem::prepareSceneGraph(QSGNode* oldNode, QQuickWindow* hostWindow)
{
    if (m_paletteDirty)
    {
        refreshPalette();
    ...
    m_builder.setResourceBudget(m_surfaces ? m_surfaces->sharedResourceBudget() : nullptr);
...
int LoopCanvasItem::syncTiles(QSGNode* root, qreal pixelScale)
{
...
int LoopCanvasItem::syncOverlays(QSGNode* root, qreal pixelScale)
{
    const pdf::PDFRevisionIdentity currentRevision =
        m_surfaces ? m_surfaces->currentRevision() : pdf::PDFRevisionIdentity();
...
void LoopCanvasItem::syncTraceHud(QSGNode* root,
...
QSGNode* LoopCanvasItem::updatePaintNode(QSGNode* oldNode, UpdatePaintNodeData*)
{
