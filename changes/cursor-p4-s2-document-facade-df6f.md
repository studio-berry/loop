...
Audience: developers
Breaking-Change: no
Summary: Put the first behaviour into the P4-S1 seam. LoupeLibInteraction gains DocumentFacade,
...
The command catalog extends docs/loupe-shell-actions.json in place rather than forking a
...
Fixes a latent LoupeLibCore defect this slice is the first to hit. The owning overload
...
Also: docs/schemas/loupe-shell-actions.schema.json describes the new optional `command`
object (additive, schema_version stays 1); architecture invariant I22 binds the rule to
UnitTestsDocumentFacade; docs/LOUPE_SHELL_CONTRACT.md documents the descriptor fields and
the DocumentFacade-to-status_contract.document projection. No behaviour is removed from
LoupeLibWidgets or LoupeLibGui -- the Widgets path keeps working as the Phase 4 migration
oracle -- and no installed artifact changes.

Phase 5 inherits one thing from this slice: loupe-shell-actions.json derives its ID set from
LoupeLibGui/pdfeditormainwindow.ui, so when that form is deleted the parity check in
verify-loupe-shell-contract.ps1 loses its source and the file must become self-authoritative.
